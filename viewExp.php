...
echo "</table>";
       ...
       echo "<table class='memo-details' border='1'>";
        // Fetch data from the imprest_action table
$query = "SELECT * FROM imprest_action WHERE imprest_id = ?";
...
while($row = $result->fetch_assoc()) {
    echo "<tr>";
    echo "<td style='padding: 3px; border-bottom: 1px solid #dddddd;'>".$row['Position_name']."</td>";
...
echo "</style>";
        ...
        if(isset($_SESSION['Position_name'])) {
            $Position_names = $_SESSION['Position_name'];
        }
...
$imprest_id = $memo['imprest_id']; // Replace with the actual memo ID variable
$position_name = $_SESSION['Position_name']; // Assuming Position_name is stored in the session

// Query to check if the Position_name already exists for the specific memo_id
$query = "SELECT * FROM imprest_action WHERE imprest_id = ? AND Position_name = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("is", $imprest_id, $position_name);
$stmt->execute();
$result = $stmt->get_result();
...
*/
   ...
   echo "</div>";
    ...
    //echo "<button class='reject-button' data-memo-idre='" . $memo['imprest_id'] . "'>Reject</button>";
           ...
           echo "<div style='text-align: center; margin-top: 20px;'>";
echo "<a href='generate_expenditure_imprest_pdf.php?imprest_id=$imprest_id' class='print-button' style='display: inline-flex; width: 40px; height: 40px; border-radius: 50%; background-color: #f0f0f0; align-items: center; justify-content: center;' title='Print'>
...
<div id="myModal" class="modal">
  <div class="modal-content">
    ...
    <button id="sendCommentButton" class="send-comment-button" style="background-color: lightgreen;">Approve</button>
  </div>
</div>
...
<div id="myModalr" class="modalr">
  <div class="modal-content">
    ...
    <button id="sendCommentButtonr" class="send-comment-button" style="background-color: lightcoral;">Reject</button>
  </div>
</div>
