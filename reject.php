...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($stmt_increment) {
        $stmt_increment->bind_param("si", $username, $memoId);
    ...
    // 5. Update the memo status to 'approved'
    $sql_update_memo = "UPDATE memos SET status = 'approved' WHERE id = ?";
    $stmt_update_memo = $conn->prepare($sql_update_memo);
    if ($stmt_update_memo) {
        $stmt_update_memo->bind_param("i", $memoId);
    ...
    // 6. Insert details into memo_action table
    $sql_insert_memo_action = "INSERT INTO memo_action (memo_id, username, status, comment, signature_path, Position_name) VALUES (?, ?, 'declined', ?, ?, ?)";
    $stmt_insert_memo_action = $conn->prepare($sql_insert_memo_action);
    if ($stmt_insert_memo_action) {
        $stmt_insert_memo_action->bind_param("issss", $memoId, $username, $comment, $signature_path, $position_name);
...
if ($from_email_notify) {
    ...
    if (!empty($next_user)) {
        ...
        if ($next_user_email) {
            ...
            $message = "Dear $next_user,<br><br>A new memo (ID: $memoId) requires your attention. Please log in to review and take action.<br><br>Best regards,<br>KCBL MEMOS System";
