...
<head>
    ...
    <title>REQUEST FOR SAFARI IMPREST</title>
    <link rel="stylesheet" type="text/css" href="safari_styles.css">
...
<body>
    ...
    <table class="table-container">
        <tr>
            ...
            <th>Date To</th>
            <th>Days</th>
            <th>Rate of Subsistance</th>        
        </tr>
        <tr>

                <td>
    <!--select id="regionDropdown" name="travelling_to" onclick="populateRegions()" id="selectedRegion" onchange="displaySelectedRegion()" required>
        <option value="" disabled selected></option>
    </select></br>
    <span id="selectedRegion"></span-->
    <input type="text" name="travelling_to" placeholder="Enter Place" required>
</td>

               
                <td><input type="date" id="dateFrom" name="Date_from" placeholder="Enter Date From" onchange="calculateDays()" required></td>
                
                <td><input type="date" id="dateTo" name="Date_to" placeholder="Enter Date To" onchange="calculateDays()" required></td>
                
                <td><input type="number" id="daysoftravell" name="Days" readonly required></td>
            <td>
    <select id="rateDropdown" onclick="populateRates()" onchange="displaySelectedRates()" placeholder="Enter the amount in Tshs">
    <input type="text" name="Rate_of_Subsistence" id="selectedRateInput" readonly>
    </select><br>
    ...
    <h3>Purpose of Safari Imprest</h3>
<!-- Textarea to store TinyMCE content -->
<textarea id="editor-safari" name="imprest_safari_purpose" rows="10" cols="80" placeholder="Please provide purpose of safari imprest">
    <?php echo isset($safariPurpose['content']) ? htmlspecialchars_decode($safariPurpose['content']) : ''; ?>
</textarea>

<div class="container-signdate">
    <div>
        <h3 id="signature-trigger">Signature <i class="material-icons" style="color: green;">&#xe226;</i></h3>
...
$sqlamount = "SELECT SUM(outstanding_imprest_amount) as total_outstanding FROM imprest_safari WHERE username = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $imprest_safari_purpose = $_POST['imprest_safari_purpose'];
    $date = $_POST['date'];
    ...
    $Days = $_POST['Days'];
    $Rate_of_Subsistence = $_POST['Rate_of_Subsistence'];
    ...
    if ($outstanding_imprest_amount > 0) {
        ...
        $sql = "INSERT INTO imprest_safari (username, position_name, department_name, imprest_amount, branch_name, signature_path, imprest_safari_purpose, date, outstanding_imprest_amount, travelling_to, Date_from, Date_to, Days, Rate_of_Subsistence, bind_memo, approver1, approver2, approver3) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ssssssssisssssssss",
            $username, $position_name, $department_name, $imprest_amount, $branch_name, $signature_path, $imprest_safari_purpose, $date, $outstanding_imprest_amount, $travelling_to, $Date_from, $Date_to, $Days, $Rate_of_Subsistence, $bind_memo, $approver1, $approver2, $approver3);
...
document.getElementById('memo-form').addEventListener('submit', function(event) {
    ...
    fetch('create_safari_imprest.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
...
function showInputFieldPopup1() {
    const contentf = `
        <div class="input-group">
            ...
            <br>
            <input type="text" name="inputField" id="inputField_1" placeholder="Choose Your Officer">
            <br>
...
function addNewDropdown1() {
    ...
    newDropdown.innerHTML = `
        <input type="text" id="inputField_${index}" class="new-input" placeholder="Choose Your Officer">
   ...
   // Function to calculate the number of days between two dates
    function calculateDays() {
        ...
        if (dateFrom && dateTo) {
            ...
            var daysDifference = (timeDifference / (1000 * 3600 * 24)) + 1; // Adding 1 to include the end date

            // Update the days of travel input field
            document.getElementById("daysoftravell").value = daysDifference;
        } else {
            document.getElementById("daysoftravell").value = 0;
        }
    }
       
</script>
<script>
// Function to populate the dropdown with rates
function populateRates() {
    var rates = ["100,000/=", "150,000/=", "230,000/=", "300,000/=", "350,000/=", "300,000/="];
...
// Function to display the selected rate
function displaySelectedRates() {
    var selectedRate = document.getElementById("rateDropdown").value;
    document.getElementById("selectedRateInput").value = selectedRate;
}

// Ensure the rates are populated when the page loads
